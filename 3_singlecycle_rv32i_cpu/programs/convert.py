...
def extract_first_hex_token(line):
    ...
    # Try explicit 0x match first
    m = HEX_PATTERN_WITH_0X.search(line)
    if m:
        return m.group(1)
    # Otherwise try first token that looks like hex (up to 8 hex chars)
    m2 = HEX_TOKEN_PATTERN.search(line)
    if m2:
        # ensure token isn't just a decimal number with letters? pattern already restricts to hex chars
        return m2.group(1)
    return None

def word_to_le_bytes_hex(word_hex):
    """Convert a hex word string to list of 2-char byte hex strings in little-endian order."""
    # normalize to even length and up to 8 chars
    word_hex = word_hex.lstrip().rstrip()
    if len(word_hex) > 8:
        # If longer than 8, take lowest 8 hex digits (least significant)
        word_hex = word_hex[-8:]
    word_hex = word_hex.zfill(8)  # pad to 8 hex digits (32 bits)
    # convert to integer then to 4 bytes little-endian
    val = int(word_hex, 16)
    b = val.to_bytes(4, byteorder='little', signed=False)
    # produce two-digit lower-case hex strings
...
def convert_file(infile, outfile):
    ...
    with open(infile, 'r') as inf, open(outfile, 'w') as outf:
        for lineno, raw in enumerate(inf, start=1):
            ...
            if not line:
                continue  # skip empty
            # skip pure comment lines that start with // or #
            if line.startswith('//') or line.startswith('#'):
                continue
            token = extract_first_hex_token(line)
            if token is None:
                # nothing hex-like on this line; skip
                continue
            # produce 4 bytes little-endian and write each as its own line
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python3 words_to_bytes_le.py <input_txt> <output_bytes_le.hex>")
        sys.exit(2)
    infile = sys.argv[1]
    outfile = sys.argv[2]
